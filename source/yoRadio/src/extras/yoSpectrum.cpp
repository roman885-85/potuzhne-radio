  ...
  if(live){
    ...
    for(uint8_t b = 0; b < n; b++){
      ...
      /*  синус повної шкали дає |X| = N/4 (вікно Ганна).
          ...
          енергія спадає до верхів, і без нахилу низи й середина стояли б під стелею.  */
      const float amp = sqrtf(p) / (N / 4.0f);
      dbs[b] = 20.0f * log10f(amp + 1e-9f) + 4.0f * log2f(fc / 1000.0f);
      if(dbs[b] > fm) fm = dbs[b];
    }
    /*  Шкала йде за гучністю: радіостанції стискають звук, і в сталій шкалі
        він стояв смугою на одній висоті — скачки ледь видно. Тут верх шкали —
        недавній найгучніший відлік (угору за чверть секунди, униз 5 дБ/с),
        низ — на 30 дБ нижче. Найгучніша смуга на піку сягає ~83 %, решта —
        нижче на свою різницю, тож до стелі нічого не липне.  */
    if(!_refOk){ _ref = fm; _refOk = true; }
    else if(fm > _ref){ float a = dt / 0.25f; if(a > 1) a = 1; _ref += (fm - _ref) * a; }
    else _ref -= decay * dt;
    if(_ref < -42.0f) _ref = -42.0f;        /* тихе не підсилюємо до шуму */
    for(uint8_t b = 0; b < n; b++){
      float l = (dbs[b] - (_ref - range)) / (range + head);
