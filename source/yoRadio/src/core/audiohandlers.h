...
void audio_error(const char *info) {
  /*  Англійське повідомлення бібліотеки — лише в журнал; на екран —
      українською, як і весь інтерфейс.  */
  telnet.printf("##ERROR#:\t%s\n", info);
  player.setError("Не вдалося з'єднатися");
}

void audio_id3artist(const char *info){
  /*  Проповідь із сайту: у шапці — її назва, а не тег виконавця з файлу  */
  if(player.remoteStationName) return;
...
void audio_id3album(const char *info){
  if(player.lockOutput || player.remoteStationName) return;
  if(printable(info)){
