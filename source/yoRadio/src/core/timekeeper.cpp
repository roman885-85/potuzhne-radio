    ...
    client->onData([](void * arg, AsyncClient * c, void * data, size_t len){
      ...
      if (bodyStart != NULL) {
        ...
        if(!result) return;
          ...
          nextion.putcmd(cmd);
          int iconofset = timekeeper.weatherIcon;
          nextion.putcmd("cond_img.pic", 50+iconofset);
          nextion.weatherVisible(1);
        #endif
        
        /*  Умову розбираємо завжди: раніше вона рахувалась лише всередині
            гілки для Nextion, і на нашому екрані значок був один на всі
            випадки життя.  */
        {
          uint8_t ic;
          if(strstr(icon,"01")!=NULL)      ic = 0;
          else if(strstr(icon,"02")!=NULL) ic = 1;
          else if(strstr(icon,"03")!=NULL) ic = 2;
          else if(strstr(icon,"04")!=NULL) ic = 3;
          else if(strstr(icon,"09")!=NULL) ic = 4;
          else if(strstr(icon,"10")!=NULL) ic = 5;
          else if(strstr(icon,"11")!=NULL) ic = 6;
          else if(strstr(icon,"13")!=NULL) ic = 7;
          else if(strstr(icon,"50")!=NULL) ic = 8;
          else                             ic = 9;
          timekeeper.weatherIcon = ic;
          timekeeper.weatherTemp  = tempf;
          timekeeper.weatherPress = (int16_t)press;
          timekeeper.weatherHum   = (int16_t)hum;
          timekeeper.weatherHave  = true;
        }
