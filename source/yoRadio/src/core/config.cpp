...
bool Config::_isFSempty() {
  /*  Сторінка тепер наша — два файли. Перевірка на старі файли yoRadio після
      їх видалення вважала SPIFFS порожнім: головна віддавала завантажувач,
      а список станцій не читався зовсім.  */
  const char* reqiredFiles[] = {"app.js.gz", "app.css.gz"};
  const uint8_t reqiredFilesSize = 2;
  char fullpath[28];
...
void Config::init() {
  ...
  #endif
  /*  Ім'я в мережі від yoRadio («yoradio-…») міняємо на своє. Автоматичне
      лише — назву, яку власник дав сам, не чіпаємо.  */
  if(!strncmp(store.mdnsname, "yoradio", 7)){
    char nm[MDNS_LENGTH]; snprintf(nm, MDNS_LENGTH, "potuzhne-%x", (unsigned int)getChipId());
    saveValue(store.mdnsname, nm, MDNS_LENGTH);
  }
  _bootDone=false;
  setTimeConf();
}
...
void Config::_setupVersion(){
  ...
  switch(currentVersion){
    case 1:
      ...
      break;
    case 2:
      snprintf(tmpBuf, MDNS_LENGTH, "potuzhne-%x", (unsigned int)getChipId());
...
bool Config::prepareForPlaying(uint16_t stationId){
  ...
  setTitle(getMode()==PM_WEB?LANG::const_PlConnect:"[наступний трек]");
  station.bitrate=0;
...
void Config::setSDpos(uint32_t val){
  if (getMode()==PM_SDCARD){
    ...
    if(!player.isRunning()){
      ...
      /*  повзунок сторінки ходить від sd_min до sd_max — це вже позиція у файлі;
          yoRadio віднімав sd_min ще раз, і перемотка лягала раніше на розмір тегів  */
      /*  Через чергу плеєра: перемотка мусить статись у головному циклі, між
          кроками декодування, а не в задачі, що прийняла запит із мережі.  */
      player.sendCommand({PR_SEEK, (int)val});
    }
  }
}
#if IR_PIN!=255
...
void Config::resetSystem(const char *val, uint8_t clientId){
  if (strcmp(val, "system") == 0) {
    ...
    saveValue(&store.telnet, true);
    saveValue(&store.watchdog, true);
    snprintf(store.mdnsname, MDNS_LENGTH, "potuzhne-%x", (unsigned int)getChipId());
...
void Config::setDefaults() {
  ...
  store.vumeter=true;   /* індикатор рівня увімкнено: без цього get_VUlevel() завжди 0 */
  store.softapdelay=0;
...
void Config::saveIR(){
  ...
  #define EEPROM_FLUSH_DELAY 10000   /* тиша перед записом, мс */
#endif

/*  Один запис після того, як зміни вщухли. Плата за це — при раптовому
    знеструмленні можна втратити останні налаштування; за збереження ресурсу
    пам'яті це прийнятно.  */
...
void Config::setStation(const char* station) {
  /*  audio_showstation() без назви від потоку передає сюди саму ж назву
      станції. memset спершу її стирав, і копіювалась уже порожнеча — під
      час проповіді з сайту шапка ставала порожньою.  */
  if(station == config.station.name) return;
...
bool Config::loadStation(uint16_t ls) {
  ...
  if (cs == 0) {
    ...
    strncpy(station.name, "ПОТУЖНЕ РАДІО", BUFLEN);
    station.ovol = 0;
    return false;
  }
...
void Config::setBrightness(bool dosave){
  ...
  extras.setBlank(false);
  extras.pwmNow();          /* яскравість знає модуль доповнень: уночі — нічна */
  if(dosave){
...
void Config::bootInfo() {
  BOOTLOG("************************************************");
  BOOTLOG("*          ПОТУЖНЕ РАДІО %s (%s)          *", prVersion(), prBuild());
  BOOTLOG("************************************************");
  BOOTLOG("------------------------------------------------");
  ...
  BOOTLOG("ir:\t\t%d", IR_PIN);
#if SD_SDMMC
  BOOTLOG("SD:\t\tSDIO %d/%d/%d/%d/%d/%d", SDMMC_CLK, SDMMC_CMD, SDMMC_D0, SDMMC_D1, SDMMC_D2, SDMMC_D3);
#else
