...
char* updateError() {
  sprintf(netserver.nsBuf, "Оновлення не вдалося (помилка %d): %s", (int)Update.getError(), Update.errorString());
...
bool NetServer::begin(bool quiet) {
  ...
  /*  Сторінки yoRadio замінено нашою (app.js, app.css). Старі файли не
      лишаємо: відкрита колись вкладка чи кеш браузера показували б із них
      стару головну, хоч налаштування вже нові.  */
  {
    static const char* const OLD[] = { "/www/script.js.gz", "/www/player.html.gz", "/www/options.html.gz", "/www/style.css.gz",
      "/www/theme.css", "/www/dragpl.js.gz", "/www/updform.html.gz", "/www/irrecord.html.gz", "/www/ir.css.gz",
      "/www/ir.js.gz", "/www/logo.svg.gz" };
    for(const char* f : OLD) if(SPIFFS.exists(f)) SPIFFS.remove(f);
  }
  yoWebApiBegin(webserver);          /* JSON для нової сторінки — раніше за роздачу файлів */
  webserver.on("/", HTTP_ANY, handleIndex);
  webserver.onNotFound(handleNotFound);
  webserver.onFileUpload(handleUpload);

  /*  Не рік, а перевірка щоразу: інакше після оновлення сторінок браузер ще
      рік показував би старі — зі старим значком і старими назвами.  */
  webserver.serveStatic("/", SPIFFS, "/www/").setCacheControl("no-store");
#ifdef CORS_DEBUG
...
void handleNotFound(AsyncWebServerRequest * request) {
  ...
  if (request->method() == HTTP_GET && request->url() == "/webboard") {
    /*  сторінка на місці: оновлення в її розділі «Оновлення»; ?raw=1 дає запасний завантажувач  */
    if(!config.emptyFS && network.status == CONNECTED && !request->hasParam("raw")){ request->redirect("/#/update"); return; }
    request->send_P(200, "text/html", emptyfs_html);
    return;
  }
  Serial.print("Not Found: ");
  Serial.println(request->url());
  request->send(404, "text/plain; charset=utf-8", "Не знайдено");
}
