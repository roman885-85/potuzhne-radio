...
class  psFrameBuffer : public Adafruit_GFX {
  public:
    ...
    /*  Штатний fillRect у Adafruit_GFX кладе по пікселю через drawPixel —
        це 76 800 викликів на повний кадр і близько 50 мс. Тут пишемо
        рядками, суцільним циклом.  */
    void fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) override {
      ...
      for(int16_t r = 0; r < h; r++){
        uint16_t* p = buffer + (size_t)(y + r) * _ww + x;
        for(int16_t c = 0; c < w; c++) *p++ = color;
      }
    }
    void fillScreen(uint16_t color) override {
      if(!buffer) return;
      uint16_t* p = buffer;
      size_t n = (size_t)_ww * _hh;
      while(n--) *p++ = color;
    }
    void display(){
      if(!buffer) return;
