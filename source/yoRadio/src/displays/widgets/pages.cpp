...
void Page::loop() {
  if(!_active) return;
  for (const auto& w : _widgets) w->loop();
  /*  Вкладені сторінки теж мають отримувати такт: підвал плеєра з повзунком
      гучності — саме така сторінка, і без цього його віджети ніколи не
      оживали, бо обхід закінчувався на власних віджетах.  */
  for (const auto& p : _pages) p->loop();
}
