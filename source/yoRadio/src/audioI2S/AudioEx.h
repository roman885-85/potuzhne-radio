...
#endif
/*  Вызывается из Audio::setSampleRate(). Слабое определение лежит в Audio.cpp,
 *  сильное — в драйвере внешнего кодека (например ES8311).                    */
extern void audio_samplerate_changed(uint32_t sampRate);
...
public:
    ...
    /*  Перемотка файлу з сервера: з якого байта просити (заголовок Range).
        Скидається після першого ж з'єднання.  */
    uint32_t yoRangeFrom = 0;
    uint32_t yoContentLen() { return m_contentlength; }
    ...
    /*  M4A: перемотка за таблицями stbl (див. Audio.cpp, yoM4aLocate)  */
    bool yoM4aLocate(uint32_t sec, uint32_t& pos, uint32_t& realSec);
