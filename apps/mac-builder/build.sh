#!/bin/bash
# =============================================================================
#  build.sh — збирає програму «Збірка ПОТУЖНОГО РАДІО» для Mac у готовий .app
#  Потрібні лише Command Line Tools (swiftc). Результат: ../../Програми/Збірка ПОТУЖНОГО РАДІО.app
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")"
ROOT="$(cd ../.. && pwd)"
APP_NAME="Збірка ПОТУЖНОГО РАДІО"
BUNDLE_ID="ua.potuzhne.macos.zbirka"
...
lipo -create "$OBJ/arm64" "$OBJ/x86_64" -output "$APP/Contents/MacOS/builder"

echo "▶ значок"
swiftc -O ../shared/make-icon.swift -o "$OBJ/make-icon" 2>/dev/null
"$OBJ/make-icon" "$OBJ/AppIcon.iconset" build >/dev/null
...
<plist version="1.0"><dict>
  ...
  <key>NSLocalNetworkUsageDescription</key><string>Щоб знайти ПОТУЖНЕ РАДІО у вашій мережі й оновити його прошивку.</string>
