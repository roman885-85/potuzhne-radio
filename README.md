...
| `tools/` | службові скрипти для перевірок через USB: `selftest.py` (пам'ять, цикл, мережа, кодек, мікрофон, АЧХ тракту, перевірка еквалайзера на слух), `gesturetest.py` (хлопки й стук, що радіо грає само, і музика без хибних команд), `exttest.py` (хлопок чи стук «із кімнати», цифрово домішаний до мікрофона поверх справжньої музики; `--soak=N` — скільки хибних за N секунд), `listtest.py` (дотики й прокрутка списку станцій), `roomrun.py`, `roomcheck.py`, `aectest.py`, `cmd.py`, `screenshot.py` |
